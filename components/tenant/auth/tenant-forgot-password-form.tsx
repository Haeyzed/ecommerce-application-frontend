...
export function TenantForgotPasswordForm({ userType }: TenantForgotPasswordFormProps) {
  ...
  const onSubmit = async (data: ForgotPasswordFormValues) => {
    ...
    try {
      const service = userType === "admin" ? createAdminAuthService(subdomain) : createCustomerAuthService(subdomain)
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6">
      <FieldGroup>
        <div className="flex flex-col items-center gap-1 text-center">
          ...
          <p className="text-sm text-muted-foreground">{userType === "admin" ? "Admin" : "Customer"} password reset</p>
        </div>
        ...
        <div className="text-center text-sm">
          <Link href={`/${userType}/login`} className="underline underline-offset-4">Back to login</Link>
        </div>
      </FieldGroup>
    </form>
  )
